...
function Nav() {
  ...
  const handleQueryChange = (e) => {
    ...
    videos.forEach((v) => {
      ...
      if (name.includes(ql))
        results.push({ id: v.id, name: getDisplayName(v, "video"), archivo: v.archivo, type: "Video" });
    });
  ...
  const handleSuggestionClick = (s) => {
    ...
    if (s.type === "Curso") navigate(`/Home/Cursos/${s.id}`);
      ...
      if (s.archivo) window.open(`http://localhost:3000/videos/files/${s.archivo}`, "_blank");
      else navigate(`/Home/Videos/${s.id}`);
    } else if (s.type === "Apunte") {
      if (s.archivo) window.open(`http://localhost:3000/apuntes/files/${s.archivo}`, "_blank");
      else navigate(`/Home/Apuntes/${s.id}`);
    } else if (s.type === "Ejercicio") {
      if (s.archivo) window.open(`http://localhost:3000/ejercicios/files/${s.archivo}`, "_blank");
