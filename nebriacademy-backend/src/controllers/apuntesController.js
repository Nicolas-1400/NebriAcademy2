...
// ── CONTROLADOR: apuntes ─────────────────────────────────────────────────────
// Maneja operaciones sobre apuntes: listado, subida, edición, borrado y notificaciones
...
// Devolver categorías posibles del modelo Apuntes
...
// Crear un nuevo apunte (subida a Cloudinary y notificaciones)
exports.create = async (req, res) => {
  try {
    ...
    // Resolver el `usuarioId` real del autor según su tipo y profileId
    if (tipo === "alumno") {
    ...
    // Generar un `public_id` seguro y subir el archivo a Cloudinary
    ...
    // Notificar a usuarios relacionados si el apunte pertenece a un curso
    try {
      if (cursoId) {
          const c = await Cursos.findByPk(cursoId);
          if (tipo === "profesor" || tipo === "administrador") {
            // Si lo sube un profesor/administrador, notificar a los alumnos apuntados
            const apuntados = await CursosAlumnos.findAll({ where: { cursoId, apuntado: true } });
            const notificaciones = [];
            for (const ap of apuntados) {
              // Para cada matrícula, buscamos el usuario alumno y preparamos la notificación
...
// Actualizar metadatos/archivo de un apunte existente
...
// Eliminar apunte y limpiar recursos (Cloudinary + notificaciones)
